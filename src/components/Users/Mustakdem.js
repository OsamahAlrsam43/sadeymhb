...
const Mustakdem = () => {
  ...
  const baseURLImg = "https://saediweb.herokuapp.com/ftp/uploads/";
  ...
  const deletefile = async (e) => {
    ...
    if (filename === "") {
      ...
      if (answer) {
        //   alert("تم حذف الملف");
        //cancelupdate();
        getnamefolder(namefolder, idfolder)

        axios.delete(`files/${getnamefile}`, {
          headers: {
            "Content-Type": "application/json",
          },
          data: { iduser: iduser, idfolder: idfolder },
      ...
      <div className="users_header">
        <h5>{user.username}</h5>
         <div className="dashboard_header_div">
                    <h5 style={{ cursor: "pointer" }} onClick={loguotuser} className="link_header_dashboard">تسجيل الخروج<BiLogOutCircle className="mr-1" size={25}/></h5>
      </div>
      </div>
